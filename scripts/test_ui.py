...
def to_one_hot(sementic_map):
    ...
    one_hot_ = one_hot.scatter_(0, sementic_map_.unsqueeze(0), 1.0)
    return one_hot_
...
E.load_state_dict(torch.load('./train_result/8.kface_l11_feat10_gl10_ll10_lre-5_onlysketch/ckpt/E_latest.pt', map_location=torch.device('cuda'))['model'], strict=False)
...
for label in ['only_sketch']:
    ...
    for i in pbar:
        semantic = Image.open(semantic_paths[0])
        ...
        # vis
        vis_np = np.array(Image.open(vis_paths[0]))
        ...
        vis = np.concatenate((style_img_[:,:,::-1],sketch_np_[:,:,::-1],fake_img_[:,:,::-1]),axis=1)
        os.makedirs(f'./result/{label}/',exist_ok=True)

        cv2.imwrite(f'./result/{label}/{str(count).zfill(6)}.png',vis)
        count += 1

    # pbar = tqdm(range(len(semantic_paths)),desc=f'{label}_semantic')
    # for i in pbar:
    #     semantic = Image.open(semantic_paths[i])
    #     sketch = Image.open(sketch_paths[-1])
    #     style_img = Image.open(style_img_path)
    #     style_vector = torch.tensor(np.load(style_path)).cuda()
    #     semantic_ = to_one_hot(semantic)
    #     # for i in range(19):
    #     #     cv2.imwrite(f'./test/{str(i).zfill(3)}.png',semantic_.numpy()[i]*255)
